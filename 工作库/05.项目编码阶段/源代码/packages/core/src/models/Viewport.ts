...
export class Viewport {
  ...
  getValidNodeElement(node: TreeNode): Element {
    const getNodeElement = (node: TreeNode) => {
      if (!node) return
      const ele = this.findElementById(node.id.toString())
      if (ele) {
        return ele
      } else {
  ...
  getValidNodeRect(node: TreeNode): Rect {
    if (!node) return
    const rect = this.getElementRectById(node.id.toString())
  ...
  getValidNodeOffsetRect(node: TreeNode): Rect {
    if (!node) return
    const rect = this.getElementOffsetRectById(node.id.toString())
    if (node && node === node.root && node.isInOperation) {
  ...
  getValidNodeLayout(node: TreeNode) {
    ...
    return calcElementLayout(this.findElementById(node.id.toString()))
  }
}
